...
from xmuda.common.solver.build import build_optimizer_2d, build_optimizer_3d, build_scheduler
...
def meter_avg(metric_logger, key, default=0.0):
    meter = metric_logger.meters.get(key, None)
    return float(meter.avg) if meter is not None else default
...
def train(cfg, output_dir='', run_name=''):
    ...
    logger.info('Experiment: DA | source={} -> target={}'.format(cfg.DATASET_SOURCE.TYPE, cfg.DATASET_TARGET.TYPE))
    logger.info('Model: 2D={} 3D={} | OpenVocab2D={} OpenVocab3D={}'.format(
        cfg.MODEL_2D.TYPE, cfg.MODEL_3D.TYPE, cfg.MODEL_2D.OPEN_VOCAB, cfg.MODEL_3D.OPEN_VOCAB
    ))
    if cfg.MODEL_2D.OPEN_VOCAB or cfg.MODEL_3D.OPEN_VOCAB:
        logger.info('Class names: {}'.format(tuple(cfg.MODEL_2D.CLASS_NAMES)))
        logger.info('Base idx: {} | Novel idx: {}'.format(tuple(cfg.MODEL_2D.BASE_CLASS_IDX),
                                                         tuple(cfg.MODEL_2D.NOVEL_CLASS_IDX)))
    ...
    logger.info('Trainable params | 2D: {:.2e}, 3D: {:.2e}'.format(
        count_trainable_params(model_2d), count_trainable_params(model_3d)
    ))
    logger.info('Loss weights | xm_src={:.4f}, xm_trg={:.4f}, pl={:.4f}'.format(
        cfg.TRAIN.XMUDA.lambda_xm_src, cfg.TRAIN.XMUDA.lambda_xm_trg, cfg.TRAIN.XMUDA.lambda_pl
    ))
    ...
    if tqdm is not None:
        pbar = tqdm(
            total=max_iteration - start_iteration,
            desc='Train(DA)',
            dynamic_ncols=True,
            leave=True,
        )
    for iteration in range(start_iteration, max_iteration):
        ...
        if pbar is not None:
            pbar.update(1)
            pbar.set_postfix({
                'iter': f'{cur_iter}/{max_iteration}',
                'src2d': f"{meter_avg(train_metric_logger, 'seg_loss_src_2d'):.3f}",
                'src3d': f"{meter_avg(train_metric_logger, 'seg_loss_src_3d'):.3f}",
                'trg2d': f"{meter_avg(train_metric_logger, 'pl_loss_trg_2d'):.3f}",
                'trg3d': f"{meter_avg(train_metric_logger, 'pl_loss_trg_3d'):.3f}",
                'iou2d': f"{meter_avg(train_metric_logger, 'seg_iou_2d') * 100:.1f}",
                'iou3d': f"{meter_avg(train_metric_logger, 'seg_iou_3d') * 100:.1f}",
                'lr2d': f"{optimizer_2d.param_groups[0]['lr']:.2e}",
                'lr3d': f"{optimizer_3d.param_groups[0]['lr']:.2e}",
            })
