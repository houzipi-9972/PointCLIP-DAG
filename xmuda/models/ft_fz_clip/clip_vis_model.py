...
class CLIPVisionTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            checkpoint = torch.jit.load(pretrained, map_location='cuda').float().state_dict()
    ...
    def forward(self, x: torch.Tensor):
        ...
        # class_embedding主要借鉴了BERT模型的用于文本分类时的思想，在每一个word vector之前增加一个类别值，通常是加在向量的第一位
        # 这里增加的class_embedding是一个可以学习的参数，经过网络的不断训练，最终以输出向量的第一个维度的输出来决定最后的输出类别
        x = torch.cat(
        ...
        # 预训练好的position embedding是对原先分辨率图像的位置编码，输入分辨率不同时需要对原先的position embedding进行2D的插值处理
