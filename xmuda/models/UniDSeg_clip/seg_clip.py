...
def _repo_pretrained_path(filename):
    repo_root = Path(__file__).resolve().parents[3]
    return str(repo_root / 'pretrained' / filename)
...
class SegCLIP(nn.Module):
    def __init__(self, backbone_2d=None):
        ...
        if backbone_2d == 'ViT-B-16':
            pretrained_dir = _repo_pretrained_path('ViT-B-16.pt')
            self.width = 768
            ...
            pretrained_dir = _repo_pretrained_path('ViT-L-14.pt')
            self.width = 1024
            ...
            pretrained_dir = _repo_pretrained_path('sam_vit_l_0b3195.pth')
            self.width = 1024
