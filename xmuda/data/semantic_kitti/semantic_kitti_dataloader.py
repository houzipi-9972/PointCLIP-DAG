...
class SemanticKITTIBase(Dataset):
    ...
    categories = {
        ...
        # Open-vocabulary setup with a vehicle superclass in base classes.
        # For training labels, truck/car are merged into "vehicle" (coarse).
        # For evaluation labels, we keep truck/car as explicit novel classes.
        'GLOBAL_OV_VEHICLE': {
            'vegetation_terrain': ['vegetation', 'trunk', 'terrain'],
            'building': ['building'],
            'road': ['road', 'lane-marking'],
            'object': ['fence', 'pole', 'traffic-sign', 'other-object'],
            'vehicle': ['truck', 'moving-truck', 'car', 'moving-car'],
            'truck': [],
            'car': [],
        },
    }
    ...
    def __init__(self,
        ...
        if merge_classes_style:
            ...
            self.label_mapping_eval = None
            if merge_classes_style == 'GLOBAL_OV_VEHICLE':
                # Fine-grained mapping used only for evaluation/analysis.
                eval_categories = {
                    'vegetation_terrain': ['vegetation', 'trunk', 'terrain'],
                    'building': ['building'],
                    'road': ['road', 'lane-marking'],
                ...
                self.label_mapping_eval = -100 * np.ones(highest_id + 2, dtype=int)
                for cat_idx, cat_list in enumerate(eval_categories.values()):
                    for class_name in cat_list:
                        self.label_mapping_eval[self.class_name_to_id[class_name]] = cat_idx
        else:
...
class SemanticKITTISCN(SemanticKITTIBase):
    def __init__(self,
                 ...
                 train_on_base_only=False,
                 base_class_indices=tuple(),
                 output_orig=False,
                 use_color=False
                 ):
        ...
        self.train_on_base_only = train_on_base_only
        self.base_class_indices = np.array(base_class_indices, dtype=np.int64)
    ...
    def __getitem__(self, index):
        ...
        if seg_label is not None and self.label_mapping is not None:
            seg_label_eval = self.label_mapping_eval[seg_label] if self.label_mapping_eval is not None else self.label_mapping[seg_label]
            seg_label = self.label_mapping[seg_label]
        else:
            seg_label_eval = seg_label.copy() if seg_label is not None else None
        ...
        if seg_label is not None:
            ...
            if self.train_on_base_only and len(self.base_class_indices) > 0:
                base_mask = np.isin(out_dict['seg_label'], self.base_class_indices)
                out_dict['seg_label'][~base_mask] = -100
