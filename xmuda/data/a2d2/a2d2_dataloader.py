...
class A2D2SCN(A2D2Base):
    ...
    def __getitem__(self, index):
        ...
        # ones_feats = np.ones([len(idxs), 1], np.float32)
        # intensity_feats = intensity[idxs]
        # rgb_feats = out_dict['img'][:, out_dict['img_indices'][:, 0], out_dict['img_indices'][:, 1]].T
        # out_dict['feats'] = np.concatenate((ones_feats, intensity_feats, rgb_feats), axis=1)
